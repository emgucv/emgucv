    ...
    public class CameraTexture : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (cameraCount == 0)
            {
                ...
                CvInvoke.PutText(img, String.Format("{0} camera found", devices.Length),
        ...
        void Update()
        {
            if (webcamTexture != null && webcamTexture.didUpdateThisFrame)
            {
                ...
                using (Mat bgra = new Mat(new Size(webcamTexture.width, webcamTexture.height), DepthType.Cv8U, 4,
                           handle.AddrOfPinnedObject(), webcamTexture.width * 4))
                using (Mat bgr = new Mat(webcamTexture.height, webcamTexture.width, DepthType.Cv8U, 3,
                           resultHandle.AddrOfPinnedObject(), webcamTexture.width * 3))
                {
                ...
                {
                    resultTexture = new Texture2D(webcamTexture.width, webcamTexture.height, TextureFormat.RGB24,
                        false);
                }
