...
namespace Emgu.CV
{
   ...
   public class FeatureTree : UnmanagedObject
   {
      ...
      private Matrix<float> DescriptorsToMatrix(Matrix<float>[] descriptors)
      {
      ...
      /// <param name="results">The results of the best <paramref name="k"/> matched from the feature tree. A m x <paramref name="k"/> matrix. Contains -1 in some columns if fewer than k neighbors found. </param>
      /// <param name="dist">A m x <paramref name="k"/> matrix of the distances to k nearest neighbors</param>
