...
:opencv-cmake
echo on
("%CMAKE%" -G"%CMAKE_GENERATOR%" -DANDROID_ABI="%ANDROID_ABI%" -DCMAKE_TOOLCHAIN_FILE="%SOURCE_DIR%\android.toolchain.cmake" %ANDROID_TOOLCHAIN_CMAKE% -DCMAKE_MAKE_PROGRAM="%MAKE%" %* "%SOURCE_DIR%" -DOPENCV_EXTRA_MODULES_PATH="%SOURCE_DIR%\opencv_contrib\modules" -DANDROID_EXTRA_NDK_VERSIONS="-r10e" -DBUILD_SHARED_LIBS:BOOL=OFF -DBUILD_ANDROID_EXAMPLES:BOOL=OFF -DBUILD_PERF_TESTS:BOOL=OFF -DBUILD_TIFF:BOOL=OFF -DWITH_TIFF:BOOL=OFF -DWITH_IPP:BOOL=OFF -DBUILD_DOCS:BOOL=OFF -DBUILD_TESTS:BOOL=OFF -DBUILD_WITH_DEBUG_INFO:BOOL=OFF -DBUILD_opencv_java:BOOL=OFF -DBUILD_opencv_ts:BOOL=OFF -DBUILD_opencv_bioinspired:BOOL=OFF -DBUILD_opencv_androidcamera:BOOL=OFF -DBUILD_opencv_adas:BOOL=OFF -DBUILD_opencv_dnn:BOOL=OFF -DBUILD_opencv_saliency:BOOL=OFF -DBUILD_TBB:BOOL=OFF -DWITH_OPENMP:BOOL=ON -DOpenMP_CXX_FLAGS="-fopenmp" -DWITH_OPENCL:BOOL=ON -DWITH_CUDA:BOOL=OFF -DCMAKE_CONFIGURATION_TYPES:STRING=Release) && GOTO cmakefin
