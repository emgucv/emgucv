...
if [ \( "$1" != "simulator" \) -a \( "$1" != "simulator_x86_64" \) ]; then    
    ...
    ../configure-device_xcode.sh $CV_CONTRIB_OPTION -DIOS_ARCH="armv7s" ${@:3}
    ...
    ../configure-device_xcode.sh $CV_CONTRIB_OPTION -DIOS_ARCH="armv7" ${@:3}
    ./xcodebuild_wrapper BITCODE_GENERATION_MODE=bitcode -parallelizeTargets -jobs 8 -configuration Release -target ALL_BUILD clean build
    cd ../../..

    mkdir -p platforms/ios/arm64
    cd platforms/ios/arm64
    ../configure-device_xcode.sh $CV_CONTRIB_OPTION -DIOS_ARCH="arm64" ${@:3}
    ./xcodebuild_wrapper BITCODE_GENERATION_MODE=bitcode -parallelizeTargets -jobs 8 -configuration Release -target ALL_BUILD clean build
    cd ../../..
fi
...
if [ "$1" != "simulator_x86_64" ]; then
    ...
    ../configure-simulator_xcode.sh $CV_CONTRIB_OPTION -DIOS_ARCH="i386" -DBUILD_IPP_IW:BOOL=FALSE -DWITH_IPP:BOOL=FALSE ${@:3}
...
#skip the first two parameter    
../configure-simulator_xcode.sh $CV_CONTRIB_OPTION -DIOS_ARCH="x86_64" -DBUILD_IPP_IW:BOOL=FALSE -DWITH_IPP:BOOL=FALSE ${@:3}
