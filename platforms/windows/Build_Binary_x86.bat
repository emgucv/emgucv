...
SET VTK_INSTALL_DIR=%INSTALL_FOLDER:\=/%/lib/cmake/vtk-9.6
...
IF %DEVENV%==%VS2026% SET GENERAL_CMAKE_CONFIG_FLAGS=%GENERAL_CMAKE_CONFIG_FLAGS% -DCMAKE_SYSTEM_VERSION:String="10.0.18362.0"
...
SET EMGU_CV_CMAKE_CONFIG_FLAGS=%EMGU_CV_CMAKE_CONFIG_FLAGS% -DBUILD_opencv_calib:BOOL=FALSE -DBUILD_opencv_dnn:BOOL=FALSE -DBUILD_opencv_photo:BOOL=FALSE -DBUILD_opencv_features:BOOL=FALSE -DBUILD_opencv_video:BOOL=FALSE
:END_CONFIG_MINI
...
for /d %%i in ( "%VS2026_DIR%\VC\Tools\MSVC\*" ) do SET VS2026_CUDA_HOST_COMPILER=%%i\bin\Hostx64\x64\cl.exe
IF %DEVENV%==%VS2026% SET CUDA_HOST_COMPILER=%VS2026_CUDA_HOST_COMPILER%
...
REM prefer CUDA 13.2
IF EXIST "%CUDA_PATH_V13_2%" SET CUDA_SDK_DIR=%CUDA_PATH_V13_2%

IF NOT EXIST "%CUDA_SDK_DIR%" SET CUDA_SDK_DIR=%CUDA_PATH_V13_2%
...
IF "%CUDA_SDK_DIR%" == "%CUDA_PATH_V13_2%" SET CUDA_ARCH_BIN_OPTION="7.5 8.0 8.6 8.9 9.0 12.0"
...
-DWITH_ONNXRUNTIME:BOOL=ON ^
-DDOWNLOAD_ONNXRUNTIME_GPU:BOOL=ON
...
IF NOT EXIST %BUILD_FOLDER% mkdir %BUILD_FOLDER%
cd %BUILD_FOLDER%
%CMAKE% -G %CMAKE_CONF% %GENERAL_CMAKE_CONFIG_FLAGS% -DENABLE_JS:BOOL=OFF -DENABLE_SAMPLES:BOOL=OFF ..
