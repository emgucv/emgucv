...
#region System.Drawing.Point example
        ...
        public void TestCvNamedWindow1()
        {
            ...
            {
                //Create the font
                //MCvFont f = new MCvFont(CvEnum.HersheyFonts.Complex, 1.0, 1.0);
...
#region Set color thresholds values
        ...
        public void TestBlobTracking()
        {
           ...
           //MCvFont font = new MCvFont(Emgu.CV.CvEnum.HersheyFonts.Simplex, 1.0, 1.0);
           ...
           using (Capture capture = new Capture())
           {
              capture.ImageGrabbed += delegate(object sender, EventArgs e)
              {
                 ...
                 foreach (MCvBlob blob in tracker)
                 {
                    ...
                    img.Draw(blob.ID.ToString(), Point.Round(blob.Center), CvEnum.HersheyFonts.Simplex, 1.0, new Bgr(255.0, 255.0, 255.0));
                 }
                 viewer.Image = img;
              };
              capture.Start();
        ...
        {
            //MCvFont font = new MCvFont(Emgu.CV.CvEnum.HersheyFonts.Simplex, 0.5, 0.5);
                ...
                capture.ImageGrabbed += delegate (object sender, EventArgs e)
                {
                    ...
                    foreach (KeyValuePair<uint, CvTrack> pair in tracks)
                            ...
                            if (pair.Value.Inactive == 0) //only draw the active tracks.
                        {
                                ...
                                result.Draw(pair.Key.ToString(), pair.Value.BoundingBox.Location, CvEnum.HersheyFonts.Simplex, 0.5, color);
