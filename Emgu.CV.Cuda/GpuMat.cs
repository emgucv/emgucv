...
namespace Emgu.CV.Cuda
{
   ...
   public class GpuMat : UnmanagedObject, IInputArray, IOutputArray
   {
      ...
      /// </summary>
      public GpuMat()
         :this(CudaInvoke.GpuMatCreateDefault())
      {
      }
      ...
      where TDepth : new()
   {
      ...
      public GpuMat(CvArray<TDepth> arr)
         : base(CudaInvoke.GpuMatCreateFromArr(arr))
      {
      }
      #endregion
      ...
      {
         CudaInvoke.GpuMatUpload(_ptr, arr);
      }
      ...
      public void Download(CvArray<TDepth> arr)
      {
         ...
         CudaInvoke.GpuMatDownload(_ptr, arr);
      }
      ...
      public void SplitInto(GpuMat<TDepth>[] gpuMats, Stream stream)
      {
         ...
         if (NumberOfChannels == 1)
         {
            ...
            CudaInvoke.Copy(_ptr, gpuMats[0], IntPtr.Zero, stream);
         }
         else
         {
            ...
            CudaInvoke.Split(_ptr, handle.AddrOfPinnedObject(), stream);
            handle.Free();
         }
      }
      ...
      public void MergeFrom(GpuMat<TDepth>[] gpuMats, Stream stream)
      {
         ...
         if (NumberOfChannels == 1)
         {
            CudaInvoke.Copy(gpuMats[0].Ptr, _ptr, IntPtr.Zero, stream);
         }
         ...
         GCHandle handle = GCHandle.Alloc(ptrs, GCHandleType.Pinned);
         CudaInvoke.Merge(handle.AddrOfPinnedObject(), _ptr, stream);
         handle.Free();
      }
         ...
         where TOtherDepth : new()
      {
         ...
         CudaInvoke.ConvertTo(Ptr, res.Ptr, 1.0, 0.0, stream);
         return res;
      }
      ...
      public GpuMat<TDepth> Reshape(int newCn, int newRows)
      {
         ...
         CudaInvoke.GpuMatReshape(this, result, newCn, newRows);
         return result;
      }
